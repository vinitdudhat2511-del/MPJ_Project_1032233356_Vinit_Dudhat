...
public class DoctorDAO {
	...
	// get count of doctors per specialization (for chart)
	public java.util.Map<String, Integer> getDoctorsPerSpecialist() {
		java.util.Map<String, Integer> map = new java.util.LinkedHashMap<String, Integer>();
		try {
			String sql = "select specialist, count(*) as cnt from doctor group by specialist order by cnt desc";
			PreparedStatement pstmt = this.conn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				map.put(rs.getString("specialist"), rs.getInt("cnt"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
